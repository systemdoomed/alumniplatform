...
function createUser($conn, $firstname, $lastname,$mail,$isSendMail, $pwd1, $course, $gradyear, $school,$gender, $title, $insta, $facebook, $xing, $linkedin, $isSupportingMember,$company2,$furtherinformation,$isSameCompany,$isDifferentCompany,$isFreelancer,$isFederal,$isFurtherEducation,$isForeignCountry,$isWorkSeeking,$birthname){
	
	$state = 1;

	$sql = "INSERT INTO Nutzer (firstname,lastname,course,school,gradyear,mail,state,isSendMail,gender,isSupportingMember,title,birthname) VALUES(?,?,?,?,?,?,?,?,?,?,?,?);";
	...
	mysqli_stmt_bind_param($stmt,"ssssssiisiss",$firstname,$lastname,$course,$school,$gradyear,$mail,$state,$isSendMail,$gender,$isSupportingMember,$title,$birthname);
	mysqli_stmt_execute($stmt);
	...
	insertLinks($conn,$nid,"",$insta,$xing,$linkedin);
