...
<ul class="navbar-nav">
<?php
echo "<li class='nav-item'><a class='nav-link' href='contactcards.php'>Kartenlayout auswählen</a></li>";
echo '</ul>';
echo '</nav>';
echo '<div class="jumbotron seamless gradient-2">';
      ...
      if($status!=0)
          {
              if($status!=1)
              {
                  ...
                  else
                  {
                      ...
                      $rumpf="SELECT N.nid, gradyear, course, firstname, lastname, company, position, mail, twitter, instagram, xing, linkedIn, school, others, facebook, showname, state, isSendMail, isExternalLinks  FROM (Nutzer N INNER JOIN Statenames S ON N.state=S.stateID) LEFT JOIN ExternalLinks E on N.nid = E.nid";
                          ...
                          if($status==3)
                              {
                                  ...
                                  $course = "%".$_GET["parameter7"]."%";
                                      $where="WHERE firstname LIKE ? AND lastname LIKE ? AND cast(gradyear As varchar(20) ) LIKE ? AND course LIKE ? AND company LIKE ? AND position LIKE ?";
                                ...
                                for ($i = 0; $i < $count; $i++)
                                    {
                                        ...
                                        for ($j = 1; $j < 16; $j++)
                                        {
                                            ...
                                            if($j==7)
                                            {
                                                if($row["isSendMail"]==0)
                                                {
                                                    echo "<td class='td text-center'></td>";
                                                }
                                                else
                                                {
                                            ...
                                            {
                                                if(($j==8)||($j==9)||($j==10)||($j==11)||($j==13)||($j==14))
                                                {
                                                    if($row["isExternalLinks"]==0)
                                                    {
                                                        echo "<td class='td text-center'></td>";
                                                    }
                                                    else
