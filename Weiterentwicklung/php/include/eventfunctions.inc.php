...
{
    $sql = "SELECT course, gradyear FROM Nutzer GROUP BY course, gradyear ORDER BY gradyear DESC, course ASC";
...
function ishistoricEvent($conn,$eid)
{
            $sql = "SELECT enddate FROM Event WHERE eid = ? AND enddate < CURDATE()";
            $stmt = mysqli_stmt_init($conn);
            $result = mysqli_query($conn, $sql);
            mysqli_stmt_prepare($stmt,$sql);
            mysqli_stmt_bind_param($stmt,"i",$eid);
