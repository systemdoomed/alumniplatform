...
function createUser($conn, $firstname, $lastname,$mail,$isSendMail, $pwd1, $matrikel, $course, $gradyear, $school, $phone,$gender, $address, $city, $company , $position, $title, $twitter, $insta, $facebook, $xing, $linkedin, $isSupportingMember,$company2,$furtherinformation,$isSameCompany,$isDifferentCompany,$isFreelancer,$isFederal,$isFurtherEducation,$isForeignCountry,$isWorkSeeking,$birthname){
	...
	$sql = "INSERT INTO Nutzer (firstname,lastname,course,school,gradyear,mail,state,isSendMail,matrikel,address,city,phone,company,position,gender,isSupportingMember,title,birthname) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
	...
	mysqli_stmt_bind_param($stmt,"ssssssiisssssssiss",$firstname,$lastname,$course,$school,$gradyear,$mail,$state,$isSendMail,$matrikel,$address,$city,$phone,$company,$position,$gender,$isSupportingMember,$title,$birthname);
	mysqli_stmt_execute($stmt);
