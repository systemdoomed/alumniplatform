...
function updateUser($conn, $nid,$firstname, $lastname,$isSendMail, $pwd1, $matrikel, $phone,$gender, $address, $city, $company , $position, $title, $twitter, $insta, $facebook, $xing, $linkedin, $isSupportingMember,$state,$others,$birthname,$isExternalLinks){

	$hashedPwd = password_hash($pwd1, PASSWORD_DEFAULT);

	$sql = "UPDATE Nutzer SET firstname = ?, lastname = ?, isSendMail = ?, matrikel = ?, phone = ?, gender = ?, address = ?, city = ?, company = ?, position = ?, title = ?, isSupportingMember = ?, state = ? , birthname = ?, isExternalLinks = ? where nid = ? ;";
	...
	mysqli_stmt_bind_param($stmt,"ssissssssssiissi",$firstname,$lastname,$isSendMail,$matrikel,$phone,$gender,$address,$city,$company,$position,$title,$isSupportingMember,$state,$birthname,$isExternalLinks,$nid);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_close($stmt);
	...
	updateLinks($conn,$twitter,$insta,$facebook,$xing,$linkedin,$others,$nid);
...
function updateLinks($conn,$twitter,$instagram,$facebook,$xing,$linkedIn,$others,$nid){
	
	$sql = "UPDATE ExternalLinks SET twitter = ?, instagram = ?, xing = ?, linkedIn = ?, facebook = ?, others = ? where nid = ?;";
	...
	mysqli_stmt_bind_param($stmt,"ssssssi",$twitter,$instagram,$xing,$linkedIn,$facebook,$others,$nid);
	mysqli_stmt_execute($stmt);
